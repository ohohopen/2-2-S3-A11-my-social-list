...
function pagiLink(e) {
	...
	if (e.target.parentElement.matches(".prev")) {
		...
		if (lowerPage <= pageNum_base) {
			for (let i = 1; i < pageNum_base + 1; i++) {
				str += `
        <li class="page-item">
          <a class="page-link pageNo" href="#" data-page="${i}">${i}</a>
        </li>
        `;
			}
			pagination.innerHTML = `
      <li class="page-item prev disabled">${str_prev}</li>
        ${str}
      <li class="page-item next">${str_next}</li>
      `;
			//截取對應的資料
        ...
        `;
			...
			pagination.innerHTML = `
      <li class="page-item prev">${str_prev}</li>
      ${str}
      <li class="page-item next">${str_next}</li>
      `;
			//截取對應的資料
	...
	if (e.target.parentElement.matches(".next")) {
		...
		//overPage大小於pagitem表示大於全部筆數, 就列出最後一組分頁鈕
		if (overPage > pagitem) {
			for (let i = 1; i < pageNum_base + 1; i++) {
				str += `
        ${str_li}${pagitem - pageNum_base + i}">${
					pagitem - pageNum_base + i
				}</a></li>
		`;
			}
			pagination.innerHTML = `
      <li class="page-item prev">${str_prev}</li>
      ${str}
      <li class="page-item next disabled">${str_next}</li>
      `;
...
function paginav(arr) {
	...
	//如果分頁鈕數量超過設定的pageNum_base, 則等於pageNum_base, 小於陣列筆數則等於陣列筆數
	...
	//如果實際頁數(跳鈕數)不超過pageNum_base頁, 就不顯示前後鈕
